...
int main(void)
{
	...
	for (; i < SIZE; i++)
		animals[i] = new Cat();
	std::cout << RESET;
	// const Animal *wrongcat = new WrongCat(); //Can't instantiate cuz it doesn't have sufficient functions
	// AAnimal animal(); //Can't instantiate abstract class
	...
	AAnimal *tmp = animals[1];							   //retain original pointer for animals[1] for correct free
